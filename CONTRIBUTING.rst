...
Report bugs at https://github.com/MeteoSwiss-APN/chain_calc/issues.
...
Chain Calculator could always use more documentation, whether as part of the official Chain Calculator docs, in docstrings, or even on the web in blog posts, articles, and such.


Submit Feedback
^^^^^^^^^^^^^^^

The best way to send feedback is to file an issue at https://github.com/MeteoSwiss-APN/chain_calc/issues.
...
Ready to contribute? Here's how to set up `chain_calc` for local development.

1. Fork the `chain_calc` repo on GitHub.
2. Clone your fork locally::

    $ git clone git@github.com:your_name_here/chain_calc.git
...
.. list-table:: Structure
   ...
   * - requirements/dev-unpinned.txt
     - Unpinned top-level development requirements, including ``chain_calc`` in editable mode (and by implication its runtime dependencies).
   ...
   * - requirements/setup.txt
     - Packages required to be installed before installing ``chain_calc`` and its dependencies.
...
By default, a single executable script called chain_calc is provided.
It is created when the package is installed.
When you call it the main function in ``src/chain_calc/cli.py`` is called.
...
When the package is installed, a executable script is created in the Python's bin folder with the name ``chain_calc``.
In the above case, when a user calls ``chain_calc``, the function ``main`` in the file ``src/chain_calc/cli.py`` is called.
